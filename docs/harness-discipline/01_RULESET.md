...
> **Charter reconciliation — the machine-origin approval exception (DECIDED — see [`06`](06_AUTONOMY_AND_MACHINE_ORIGIN.md)).** `RUNTIME_CHARTER.md:96` anticipates that a *future* policy may grant "a narrowly scoped machine-origin exception" to the human-approval requirement. RL-04, RL-13, and R-26 are written as absolutes because for a self-evolving harness that runs unattended (cron, self-evolution, CUA loops) an undefined exception is an open authority hole. **Resolved** in [`06_AUTONOMY_AND_MACHINE_ORIGIN.md`](06_AUTONOMY_AND_MACHINE_ORIGIN.md): the machine-origin lane is **OFF** and gated behind an 11-item precondition set (P-1..P-11); RL-04 stays **absolute** (human-only, halt-and-queue R-08) until those are CI-green; the shipped allowlist is empty; high/critical/irreversible are *never* eligible. A 2026-06-24 red team (15 exploit paths) hardened the design — `06` documents the lane as unbuilt and the empty-allowlist as the only current safety property.
...
### RL-04 — Approval evidence for high/critical actions must come from an out-of-band human signal, never synthesized (machine-origin is a gated, honestly-labeled exception — see `06`).
...
**Amendment (machine-origin).** A **machine-origin** authorization is permitted *only* when the full precondition set in [`06_AUTONOMY_AND_MACHINE_ORIGIN.md`](06_AUTONOMY_AND_MACHINE_ORIGIN.md) §4 (P-1..P-11) is CI-green, the capability is on a structured human-ratified `AutonomyPolicyV1` allowlist, and it is recorded with `approval.source = "machine_origin"` (never impersonating a human ref). It is **never** valid for `high`/`critical`/irreversible actions. Until the preconditions are green this rule is **absolute** (human-only); the shipped allowlist is empty.
**Detection:** Schema validation — `human_confirmation` evidence_refs must carry an out-of-band provenance token (operator/CLI/Telegram approval id); reject refs whose source is an in-process confidence value. The `machine_origin` provenance (P-3) must be a distinct enum value, never valid for ≥ medium. *Status: missing.*
