...
| --- | --- | --- |
| `spark-telegram-bot` | Installed head `fe39d37c80c2` (`fe39d37c80c226c549a730e854e58dcac7aaa4af`); registry pin `e5a1bd040986`; classified `local_runtime_test_artifact`. | Push or port the proven Telegram head, including `/access 5` activation proof, the Level 5 Codex sandbox confirmation fix, effective sandbox Telegram surface proof, read-only contradiction full-access copy block, effective Level 5 sandbox-before-operator-claims guard, and Level 5 status sandbox guard, then update registry/release metadata so the runtime is no longer local-only. |
| `spawner-ui` | Installed head `97cb911bf7ac`; registry pin `19b7d0bff144`; classified `local_runtime_test_artifact`. | Push or port the proven Spawner head, including direct-client, PRD-lane, and persisted Level 5 Codex sandbox fixes, then update registry/release metadata. |
