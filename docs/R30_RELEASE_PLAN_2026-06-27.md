...
| --- | --- | --- |
| `spark-telegram-bot` | Installed head `4856174b4b7d`; registry pin `e5a1bd040986`; classified `local_runtime_test_artifact`. | Push or port the proven Telegram head, including `/access 5` activation proof, the Level 5 Codex sandbox confirmation fix, and the effective sandbox Telegram surface proof, then update registry/release metadata so the runtime is no longer local-only. |
