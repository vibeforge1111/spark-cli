...
| --- | --- | --- |
| `spark-telegram-bot` | Installed head `28aecd8ddfb` (`28aecd8ddfb49dde730926df37ba3c81c55c6617`); registry pin `e5a1bd040986`; classified `local_runtime_test_artifact`. | Push or port the proven Telegram head, including `/access 5` activation proof, the Level 5 Codex sandbox confirmation fix, effective sandbox Telegram surface proof, read-only contradiction full-access copy block, and effective Level 5 sandbox-before-operator-claims guard, then update registry/release metadata so the runtime is no longer local-only. |
