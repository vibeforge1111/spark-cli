...
| --- | --- | --- |
| `spark-telegram-bot` | Installed head `02791932e194`; registry pin `e5a1bd040986`; classified `local_runtime_test_artifact`. | Push or port the proven Telegram head, including `/access 5` activation proof, the Level 5 Codex sandbox confirmation fix, the effective sandbox Telegram surface proof, and the read-only contradiction full-access copy block, then update registry/release metadata so the runtime is no longer local-only. |
