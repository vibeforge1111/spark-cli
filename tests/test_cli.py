...
class SparkCliTests(unittest.TestCase):
    ...
    def test_approval_classifier_flags_git_worktree_discard(self) -> None:
        cases = [
            ["git", "restore", "."],
        ...
        for command in cases:
            with self.subTest(command=command):
                ...
                self.assertEqual(decision.approval_mode, "blocked")
                self.assertEqual(decision.confirmation_phrase, "approve git worktree discard")

        status = approval_required_for_command(["git", "status"], CommandContext(non_interactive=True))
