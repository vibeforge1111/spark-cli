...
"""
    ...
    def test_windows_install_script_bootstraps_local_prefix_contract(self) -> None:
        ...
        self.assertIn("$sparkCmd live start", script)
        self.assertIn("$sparkCmd live status", script)
        self.assertIn("$sparkCmd providers status", script)
        self.assertIn("$sparkCmd providers test --role chat", script)
        self.assertIn("$sparkCmd verify --onboarding", script)
        self.assertIn("$sparkCmd fix telegram", script)
        self.assertIn("$sparkCmd fix spawner", script)
        self.assertIn("$sparkCmd fix autostart", script)
        self.assertIn("choose Level 4", script)
        self.assertIn("Use a lower level only", script)
        self.assertIn("Mission Control, Kanban, Canvas, or preview links", script)
        self.assertIn("$sparkCmd autostart off", script)
        self.assertIn("$sparkCmd autostart on telegram-starter --now", script)
