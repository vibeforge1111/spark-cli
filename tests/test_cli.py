...
class SparkCliTests(unittest.TestCase):
    ...
    def test_approval_classifier_flags_kubectl_node_mutation(self) -> None:
        for command in (
            ...
            with self.subTest(command=command):
                ...
                self.assertEqual(decision.approval_mode, "blocked")
                self.assertEqual(decision.confirmation_phrase, "approve kubernetes node change")

    def test_approval_classifier_allows_kubectl_node_inspection(self) -> None:
        for command in (["kubectl", "get", "nodes"], ["kubectl", "describe", "node", "spark-worker"], ["kubectl", "top", "nodes"]):
            with self.subTest(command=command):
                decision = approval_required_for_command(command, CommandContext(non_interactive=True))
                self.assertFalse(decision.requires_approval)
                self.assertEqual(decision.action_class, "none")
