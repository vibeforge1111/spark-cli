...
class SparkCliTests(unittest.TestCase):
    ...
    def test_approval_classifier_flags_python_package_publish(self) -> None:
        for command in (
            ["twine", "upload", "dist/*"],
            ...
            with self.subTest(command=command):
                ...
                self.assertEqual(decision.approval_mode, "blocked")
                self.assertEqual(decision.confirmation_phrase, "approve python package publish")

        check = approval_required_for_command(["twine", "check", "dist/*"], CommandContext(non_interactive=True))
