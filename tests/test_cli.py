...
class SparkCliTests(unittest.TestCase):
    ...
    def test_approval_classifier_flags_kubectl_create_mutation(self) -> None:
        for command in (
            ["kubectl", "create", "namespace", "spark"],
            ["kubectl", "create", "deployment", "spark", "--image=spark:latest"],
            ["kubectl", "create", "configmap", "spark-config", "--from-literal=mode=test"],
        ):
            with self.subTest(command=command):
                ...
                self.assertEqual(decision.approval_mode, "blocked")
                self.assertEqual(decision.confirmation_phrase, "approve kubernetes create")

    def test_approval_classifier_flags_kubectl_create_credentials(self) -> None:
        for command in (
    ...
    def test_approval_classifier_allows_kubectl_create_dry_run(self) -> None:
        for command in (
