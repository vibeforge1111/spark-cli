...
class SparkCliTests(unittest.TestCase):
    ...
    def test_approval_classifier_flags_ruby_gem_publish(self) -> None:
        decision = approval_required_for_command(["gem", "push", "pkg/example-0.1.0.gem"], CommandContext(non_interactive=True))
        self.assertTrue(decision.requires_approval)
        self.assertEqual(decision.action_class, "external_publish")
        self.assertEqual(decision.risk, "high")
        self.assertEqual(decision.approval_mode, "blocked")
        self.assertEqual(decision.confirmation_phrase, "approve ruby gem publish")

        build = approval_required_for_command(["gem", "build", "example.gemspec"], CommandContext(non_interactive=True))
        self.assertFalse(build.requires_approval)
