...
def approval_required_for_command(argv: list[str], context: CommandContext | None = None) -> ApprovalDecision:
    ...
    if first in {'mount', 'umount', 'modprobe', 'insmod', 'rmmod', 'losetup', 'swapon', 'swapoff', 'kexec'}:
        return _decision(
            parts, ctx,
            'container_privilege_escalation',
            'critical',
            'Command manipulates kernel modules, filesystem mounts, or host privilege state.',
            target_display=' '.join(parts[:4]),
            confirmation_phrase='approve kernel state change',
        )
    ...
    if first in {'chmod', 'chown', 'chgrp', 'setfacl'}:
        ...
        has_sensitive = any(
            any(p == sp or p.startswith(sp + '/') for sp in sensitive_paths)
            for p in parts[1:]
        )
