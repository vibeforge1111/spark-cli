...
def approval_required_for_command(argv: list[str], context: CommandContext | None = None) -> ApprovalDecision:
    ...
    if (
        ...
        or (first == "az" and lowered[1:4] in [["keyvault", "secret", "show"], ["keyvault", "secret", "download"]])
        or (first == "op" and second in {"read", "document", "item"})
        ...
        return _decision(
            ...
            "credential_mutation",
            "critical",
            "Command can reveal cloud or password-manager secrets.",
            target_display=" ".join(parts[:4]),
            confirmation_phrase="approve cloud secret reveal",
        )
