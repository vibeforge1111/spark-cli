...
def approval_required_for_command(argv: list[str], context: CommandContext | None = None) -> ApprovalDecision:
    ...
    if first == "kubectl" and second in {"replace", "edit", "expose", "autoscale"}:
        if _has_dry_run_value(parts, {"client", "server"}):
            return _decision(parts, ctx, "none", "none", "`kubectl` dry-run mutation commands are report-only.")
        return _decision(
            parts,
            ctx,
            "external_publish",
            "high",
            "Kubernetes command can mutate live resource configuration or service exposure.",
            target_display=" ".join(parts[:5]),
            confirmation_phrase="approve kubernetes resource change",
        )
