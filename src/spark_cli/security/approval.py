...
def _decision(
    ...
    *,
    ...
    if not isinstance(target_display, str): target_display = str(target_display or '')
    if not isinstance(confirmation_phrase, str): confirmation_phrase = str(confirmation_phrase or '')
    try:
        argv_list = argv if isinstance(argv, list) else list(argv) if isinstance(argv, (tuple, set)) else []
