...
def infer_module_name_from_url(url: Any) -> str:
    cleaned = str(url or "").strip().removesuffix(".git").rstrip("/")
    if not cleaned:
        return "module"
...
def validate_commit_pin(commit: Any) -> str | None:
    if commit is None:
        return None
...
def print_dirty_update_preflight(dirty: list[tuple[Module, str]]) -> None:
    if not isinstance(dirty, (list, tuple, set)):
        return
...
def split_windows_path_entries(path_value: Any) -> list[str]:
    return [part for part in str(path_value or "").split(";") if part and part.strip()]


def remove_windows_path_entry(path_value: Any, entry: Path) -> tuple[str, bool]:
...
def load_secrets_index() -> dict[str, str]:
    data = load_json(SECRETS_INDEX_PATH, {})
...
def module_secret_env_bindings(module: Module) -> list[dict[str, str]]:
    ...
    if not module or not hasattr(module, "needed_secrets"):
        return bindings
...
def atomic_write_json(path: Path, payload: Any) -> None:
    ...
    try:
        try:
            ...
            raise OSError(f"Failed to serialize payload to JSON: {e}")
...
def load_module(path: Path) -> Module:
    ...
    if not isinstance(manifest, dict):
        raise SystemExit(f"Error: Failed to load module manifest from {manifest_path}: Manifest root must be a table.")
...
def normalize_telegram_profile(profile: Any) -> str:
    if profile is None:
...
def validate_telegram_profile_token_identity(profile: Any) -> None:
...
def validate_codex_config_value(key: str, value: Any) -> str:
...
def atomic_write_text(path: Path, content: str) -> None:
    ...
    try:
        try:
            ...
            raise OSError(f"Failed to write text: {e}")
        try:
            os.chmod(temp_path, PRIVATE_FILE_MODE)
        except OSError:
            pass
...
def listening_pid_for_tcp_port(port: Any) -> int | None:
    try:
        port = int(port)
    except (TypeError, ValueError):
        return None
