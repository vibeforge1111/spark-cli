...
def _wrap_subgroup_help(group_parser: argparse.ArgumentParser, examples: list[str]) -> None:
    """Replace argparse's terse 'arguments are required: <dest>' with a friendly,
    operator-readable listing of the valid subcommands. The exit-2 contract for
    missing-subcommand is preserved so scripting consumers are unaffected; only
    the human-facing error text changes. Applied to subcommand groups that have
    `required=True` subparsers (config, autostart, telegram, secrets, os, etc.)
    so a user typing `spark <group>` with no subcommand sees a useful list
    instead of the bare argparse 'arguments are required: <opaque_dest>' line.
    """
